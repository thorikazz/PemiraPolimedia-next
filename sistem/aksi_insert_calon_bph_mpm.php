...
if (isset($_POST['simpan'])) {
    ...
    if (in_array($ekstensi, $ekstensi_diperbolehkan) === true) {
        if ($ukuran <= 2000000) {
            move_uploaded_file($file_tmp, 'foto/bph_mpm/' . $foto);
        }
    }

    mysqli_query($koneksi, "INSERT INTO calon_bph_mpm(id, foto, nama, nim, ttl, jenis_kelamin, prodi, semester, no_urut)
    VALUES ('', '$foto', '$nama','$nim','$ttl','$jenis_kelamin','$prodi', '$semester', '$no_urut')");

    echo "<script>window.alert('Berhasil')
    window.location='input_data_calon1.php'</script>";
}
